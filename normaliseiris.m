...
b = sgn.*cos(pi - phi - theta);%theta is the angle between pupil line with x axis

% calculate radius around the iris as a function of the angle

r = (sqrt(a).*b) + ( sqrt( a.*(b.^2) - (a - (r_iris^2))));%pupil center to iris boundary
...
%disp(size(x));disp(size(y));disp(size(image));
...
addpos = pos(l);
%if Windows
%final_normal = [eyeimage_filename(1:addpos),eyeimage_filename(addpos+1:posdot),'\normal-',eyeimage_filename(addpos+1:posdot),'.jpg'];
%if Mac
final_normal = [eyeimage_filename(1:addpos),eyeimage_filename(addpos+1:posdot-1),'/normal-',eyeimage_filename(addpos+1:posdot-1),'.jpg'];

imwrite(image,final_normal,'jpg');
% 
% cd(w);
end
% end diagnostics
